...
describe("map registry", () => {
  it("exposes the hub and arena maps", () => {
    expect(maps.map((m) => m.id)).toEqual(["cinder-plaza", "rustyard-arena"]);
...
describe("rustyard-arena", () => {
  const arena = requireMap("rustyard-arena");

  it("matches the enc-rustyard-ambush combat grid size", () => {
    const encounter = getEncounter("enc-rustyard-ambush");
    expect(encounter?.grid).toEqual({ width: arena.width, height: arena.height });
  });

  it("has player and enemy spawns", () => {
    expect(arena.spawns.map((s) => s.id)).toEqual(["player", "enemy-1", "enemy-2"]);
  });
});
